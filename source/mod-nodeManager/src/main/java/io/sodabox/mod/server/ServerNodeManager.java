...
import io.sodabox.mod.Action;
import io.sodabox.mod.server.map.ConsistentHash;
import io.sodabox.mod.server.map.NodeMap;
...
public class ServerNodeManager extends AbstractNodeManager<ServerNode>{
	...
	public void messageHandle(Message<JsonObject> message) {
		...
		switch (action) {
		case Action.SERVER.SERVER_NODE:
			String refer	= message.body.getString("refer");
			sendOK(message, getServerNode(refer));
			break;
		case Action.SERVER.OK:
